        ...
        <div className='relative bg-white rounded-4xl shadow-xl shadow-purple-500 p-4 md:p-8 w-full max-w-5xl h-auto overflow-y-auto max-h-[95vh] flex flex-col lg:flex-row gap-8 items-center'>
            ...
            <div className='flex flex-col items-center w-full lg:w-1/3'>
                ...
                <div className='flex gap-4 mt-6 w-full justify-center'>
                    <button 
                        ...
                        className='flex-1 max-w-[120px] rounded-xl border-2 border-gray-200 p-2 text-white font-semibold bg-[#5c0202] hover:bg-[#940404] hover:scale-105 transition-all duration-300 shadow-md cursor-pointer'>
                        {button}
                    </button>
                    <button 
                        onClick={() => navigate(`/editar/${idCard}`)}
                        className='flex-1 max-w-[120px] rounded-xl border-2 border-gray-200 p-2 text-white font-semibold bg-purple-900 hover:bg-purple-700 hover:scale-105 transition-all duration-300 shadow-md cursor-pointer'>
                        {button2}
                    </button>
                </div>  
            </div>
            ...
            <div className='flex-1 w-full flex flex-col gap-4'>
                ...
                <div className='grid grid-cols-3 gap-2 md:gap-4'>
                    <div className='border-2 border-purple-800 rounded-xl p-2 text-center shadow-md'>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                        {listaHabilidadesIds.length > 0 ? (
                            listaHabilidadesIds.map((habId, i) => (
                                <div key={i} className='border-2 border-purple-700 rounded-xl p-2 text-center text-sm shadow-sm'>
                                    {obtenerNombreHabilidad(habId)}
                                </div>
                            ))
                        ) : (
                            <div className='col-span-3 border-2 border-dashed border-gray-300 rounded-xl p-2 text-center text-sm italic text-gray-500'>
            ...
            <button onClick={() => noMostrar2()}
                className='absolute top-4 right-4 bg-white border-2 border-gray-300 rounded-full p-1 hover:bg-gray-100 transition-all shadow-md z-10 cursor-pointer'>
